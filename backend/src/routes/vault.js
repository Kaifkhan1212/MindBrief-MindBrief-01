...
// const express = require("express");
// console.log("🔥 vault.js reached BEFORE firebase-admin");

// const admin = require("firebase-admin"); // crash happens here
// const router = express.Router();

// console.log("🔥 vault.js loaded AFTER firebase-admin");
...
if (!admin.apps.length) {
  try {
    ...
    // Check if env var exists and is not a placeholder/comment
    if (
      process.env.FIREBASE_SERVICE_ACCOUNT &&
      !process.env.FIREBASE_SERVICE_ACCOUNT.startsWith("#") &&
      !process.env.FIREBASE_SERVICE_ACCOUNT.includes("...") &&
      !process.env.FIREBASE_SERVICE_ACCOUNT.includes("(see Firebase section)")
    ) {
      try {
        // Check if it's a placeholder or non-JSON string
        ...
        if (envValue.startsWith("{") && envValue.endsWith("}")) {
          ...
          console.log("ℹ️  FIREBASE_SERVICE_ACCOUNT is present but not valid JSON. Using in-memory storage.");
        }
      } catch (parseError) {
        console.warn(
          "⚠️  Failed to parse FIREBASE_SERVICE_ACCOUNT as JSON:",
          parseError.message
        );
      }
    }
